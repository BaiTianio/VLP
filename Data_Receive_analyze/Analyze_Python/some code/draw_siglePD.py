...
def draw_all_PD():
    for PD_num in np.arange(32):
        data1=all_data["PD{}_{}".format(PD_num,1)]
        data2=all_data["PD{}_{}".format(PD_num,2)]
        data3=all_data["PD{}_{}".format(PD_num,3)]
        x=np.arange(15,80,2)
        plt.style.use("seaborn-poster")    
        fig1=plt.figure()
        plt.title("PD="+str(PD_num))
        plt.plot(x,data1,label='Times=1')
        plt.plot(x,data2,label='Times=2')
        plt.plot(x,data3,label='Times=3')
        plt.ylim((0,1500))
        plt.legend()
...
loadData=load_data()
all_data=loadData.load_AllData()
